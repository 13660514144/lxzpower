...
namespace HelperTools
{
    public class RedisHelper
    {
        ...
        //单例模式
        public RedisCommon Default { get { return new RedisCommon(DbType); } }
        //public static RedisCommon UserKey { get { return new RedisCommon(0); } }
    ...
    public class RedisCommon
    {
        ...
        //redis数据库连接字符串
        private string _conn = string.Empty;// "127.0.0.1:6379";
        public static int _db = 15;
        ...
        /// 构造函数
        /// </summary>
        /// <param name="DbTye"></param>
        public RedisCommon(string DbTye)
        {
            _conn = GetConn(DbTye);
        }
        /// <summary>
        ...
        public ConnectionMultiplexer CacheConnection
        {
            get
            {
                ...
                //catch (Exception ex)
                //{
                //    LogHelper.Error("RedisHelper->CacheConnection 出错\r\n" + ex.ToString());
                //    return null;
                //}
                return connection;
            }
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetConn(string DbTye)
        {
            string Path = System.Configuration.ConfigurationManager.AppSettings["ConFigPath"];
            string Connection = string.Empty;
            string Json = FileHelper.ReadFile($@"{Path}\ConfigSet.json");
            JObject Obj = JObject.Parse(Json);
            StringBuilder Sb = new StringBuilder();
            Sb.Append($"{Obj["Redis"]["Host"].ToString()},");
            Sb.Append($"{Obj["Redis"]["Pwd"].ToString()}");
            Connection = Sb.ToString();
            JObject DbNum = (JObject)Obj["Redis"]["DataNumber"];
            foreach (var item in DbNum)
            {
                if (item.Key.ToUpper() == DbTye.ToUpper())
                {
                    _db = Convert.ToInt16(item.Value);
                    break;
                }
            }
            return Connection;
        ...
        public string StringGet(string key)
        {
            ...
            {
                LogHelper.Error("RedisHelper->StringGet 出错\r\n" + ex.ToString());
                return string.Empty;
            }
        }
